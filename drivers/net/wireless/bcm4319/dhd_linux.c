 ...
 *
 * $Id: dhd_linux.c,v 1.65.4.9.2.12.2.104.4.11 2010/09/15 02:45:57 Exp $
 */
...
static int dhd_keep_alive_onoff(dhd_pub_t *dhd, int ka_on)
{
	...
	int str_len, buf_len;
	int res = 0;
	int keep_alive_period = 45000; /* in ms */
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend && !dhd->suspend_disable_flag) {
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	dhd_info_t *dhd = NULL;
	struct net_device *net;
	extern int check_chip;
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	/* updates firmware nvram path if it was provided as module paramters */
	if ((firmware_path != NULL) && (firmware_path[0] != '\0')) {
		if(strstr(firmware_path, "test")) {                                            
			if((check_chip & 0xffff) == 0x4319) 
                            strcpy(fw_path, test_4319_path);
			else if((check_chip & 0xffff) == 0x4329) 
                            strcpy(fw_path, test_4329_path);
		} 
		else {
			if((check_chip & 0xffff) == 0x4319)
                            strcpy(fw_path, fw_4319_path);
			else if((check_chip & 0xffff) == 0x4329)
                            strcpy(fw_path, fw_4329_path);
		}
	}
	if (1/*(nvram_path != NULL) && (nvram_path[0] != '\0')*/) {
		if((check_chip & 0xffff) == 0x4319) 
                        strcpy(nv_path, nv_4319_path);

                else if((check_chip & 0xffff) == 0x4329) 
                        strcpy(nv_path, nv_4329_path);
